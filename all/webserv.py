...
def main():
	...
	for i in lines:
		a = i.split("=")[1]
		path.append(a)
	try: # get all the parameter
		FILE = path[0]
		CGIBIN = path[1]
		EXEC = path[3]
	except IndexError: # handle not enough filed.
	...
	while True:
		...
		if pid == 0:
			...
			# handle different files in different ways.
			if "files" in path1:
...
# TODO: simple function for read a file
def read_the_static(filepath,FILE):
	...
	file.close()
	return index
# TODO: main function for handle static file.
	...
	try:
		...
		if ".html" in path:
			try:
				...
				if "gzip" in O:
					return html_content.encode() + gzip.compress(read_the_static(path,FILE).encode())
				else:
		...
		if ".txt" in path:
			try:
				O = os.environ["HTTP_ACCEPT_ENCODING"]
				if "gzip" in O:
					return txt_content.encode() + gzip.compress(read_the_static(path,FILE).encode())
				else:
		...
		if ".js" in path:
			try:
				O = os.environ["HTTP_ACCEPT_ENCODING"]
				if "gzip" in O:
					return js_content.encode() + gzip.compress(read_the_static(path,FILE).encode())
				else:
		...
		if ".css" in path:
			try:
				O = os.environ["HTTP_ACCEPT_ENCODING"]
				if "gzip" in O:
					return css_content.encode() + gzip.compress(read_the_static(path,FILE).encode())
				else:
		...
		if ".png" in path:
			try:
				O = os.environ["HTTP_ACCEPT_ENCODING"]
				if "gzip" in O:
					return png_content.encode() + gzip.compress(read_the_static(path,FILE).encode())
				else:
					return (png_content + read_the_static(path,FILE)).encode('utf-8')
			except KeyError:
		...
		if ".jpg" in path or ".jpeg" in path:
			try:
				O = os.environ["HTTP_ACCEPT_ENCODING"]
				if "gzip" in O:
					return jpg_content.encode() + gzip.compress(read_the_static(path,FILE).encode())
				else:
					return (jpg_content + read_the_static(path,FILE)).encode('utf-8')
			except KeyError:
		...
		if ".xml" in path:
			try:
				O = os.environ["HTTP_ACCEPT_ENCODING"]
				if "gzip" in O:
					return xml_content.encode() + gzip.compress(read_the_static(path,FILE).encode())
				else:
...
''').encode('utf-8')
			try:
				O = os.environ["HTTP_ACCEPT_ENCODING"]
				if "gzip" in O:
					return html_content.encode() + gzip.compress(the_default)
				else:
					return html_content.encode() + the_default
			except KeyError:
				return (html_content.encode() + the_default)
...
#TODO: main function to handle cgi program.
...
'''
		...
		if(check_succeed[1]==768):
			cgi_header = '''HTTP/1.1 200 OK
Content-Type: text/html\n
'''
			content = "<h1>Slept for some time!</h1>\n"
			return (cgi_header+content).encode('utf-8')
...
'''
		...
		try:
				O = os.environ["HTTP_ACCEPT_ENCODING"]
				if "gzip" in O:
					return cgi_header.encode() + gzip.compress(content.encode())
				else:
					return (cgi_header + content).encode()
		except KeyError:
				return (cgi_header + content).encode()
